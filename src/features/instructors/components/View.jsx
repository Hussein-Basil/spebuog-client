...
const View = ({ gridView, loading, filteredSpeakers, responsiveGridView }) => {
    ...
    >
      <Flex alignSelf="start">
        <SkeletonCircle
          ...
          as={Avatar}
        />
        <Flex flexDir="column" align="start" pl="1em">
    ...
    <>
      {responsiveGridView && gridView ? (
        ...
        >
          {loading
            ...
            ? filteredSpeakers?.slice(0, sliceIndex).map((speaker, i) => (
                <LinkBox>
                  <LinkOverlay href={`/instructor/${speaker.slug?.current}`}>
                    <Flex
                      key={speaker?._id || i}
                      flexDir="column"
                      align="center"
                    ...
                    >
                      <Flex alignSelf="start">
                        <Image
                          ...
                          src={
                            speaker.image?.asset
                              ? urlFor(speaker.image).width(100)
                              : "unset"
                          }
                          alt="instructor"
                          width="100px"
                        ...
                        />
                        <Flex flexDir="column" align="start" pl="1em">
                          <Heading fontSize="16px" w="80%" fontWeight="medium">
                            {speaker.name}
                          </Heading>
                          <Text fontSize="14px" w="80%" color="gray">
                            {speaker.position}
                          </Text>
                          ...
                          <Link
                            href={`/instructor/${speaker.slug?.current}`}
                            variant="textButton"
        ...
        <Flex flexDir="column" gap="1em" maxW={{ base: "90vw", md: "100%" }}>
          {loading
            ? nullSpeakers
            : filteredSpeakers?.length &&
              filteredSpeakers.slice(0, sliceIndex).map((speaker, idx) => {
                return (
                  <LinkBox>
                    <LinkOverlay href={`/instructor/${speaker.slug?.current}`}>
                      <Flex
      ...
      {sliceIndex < filteredSpeakers.length && (
        <Button
          mt="2em"
          variant="outline"
          onClick={() => setSliceIndex(undefined)}
        >
          Load More
        </Button>
      )}
